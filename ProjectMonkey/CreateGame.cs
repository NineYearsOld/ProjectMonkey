    ...
    class CreateGame
    {
        ...
        private Bitmap DrawForrest(int width, int height)
        {
            ...
            using (Graphics graph = Graphics.FromImage(bitmap))
            {
                ...
                for (int i = 0; i < monkeyNames.Length; i++)
                {
                    Pen randomColor = new Pen(Color.FromArgb(color.Next(0, 255), color.Next(0, 255), color.Next(0, 255)),2);
                    int launcher = 1;
                    var ape = monkeyPaths.Where(a => a.monkeyName == monkeyNames[i]);
                    foreach (var primate in ape)
                    {
                        graph.DrawEllipse(randomColor, primate.monkeyX - 7, primate.monkeyY - 7, 14, 14);
